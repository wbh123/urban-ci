...
describe('spatial map store', () => {
  beforeEach(() => {
    ...
    vi.clearAllMocks()
    mocks.querySpatialCommunities.mockResolvedValue({
      type: 'FeatureCollection',
      features: [feature('c1', 'COMMUNITY')],
    })
    mocks.querySpatialBuildings.mockResolvedValue({
      type: 'FeatureCollection',
      features: [feature('b1', 'BUILDING', 'c1'), feature('b2', 'BUILDING', 'c1')],
    })
  ...
  it('loads verified polygons by bbox and merges risk rows by building id', async () => {
    ...
    expect(store.visibleBuildings.map((item) => [item.feature.id, item.risk?.riskLevel])).toEqual([
      ['b1', 'HIGH'],
      ['b2', 'LOW'],
    ])
  })
  ...
  it('ignores stale viewport responses when a newer request finishes first', async () => {
    ...
    mocks.querySpatialBuildings
      .mockImplementationOnce(() => new Promise((resolve) => { resolveOld = resolve }))
      .mockResolvedValueOnce({ type: 'FeatureCollection', features: [feature('new', 'BUILDING', 'c1')] })
    mocks.querySpatialCommunities.mockResolvedValue({ type: 'FeatureCollection', features: [] })
