  ...
  try {
    ...
    mapMounted.value = await driver.mount(mapContainer.value, config, {
      ...
      onBuildingModelClick: (hit) => {
        ...
        void openBuildingDetail(hit.buildingId)
      },
    })
...
function syncMap(): void {
  ...
  const point = buildingId ? buildingMapPoints.value.find((item) => item.id === buildingId) : undefined
  const selection: SpatialAmapActiveBuilding | null = point
    ? {
        source: 'SYSTEM',
        buildingId: point.id,
    ...
    <div class="spatial-workspace">
      <aside class="spatial-list-panel">
        ...
        <el-scrollbar class="building-scroll">
          <button v-for="item in visibleRiskBuildings" :key="item.buildingId" type="button" class="building-row" :class="{ selected: selectedBuildingIds.includes(item.buildingId) }" @click="selectBuilding(item.buildingId)">
      ...
      <main class="map-panel">
        ...
        <div class="map-legend">
          ...
          <span>◇ 小区位置</span>
          <span>▢ 已确认边界</span>
          <span class="legend-note">半透明表示结果已过期，灰色表示暂无结果</span>
        </div>
      </main>
    </div>
