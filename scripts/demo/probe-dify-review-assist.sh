...
print(json.dumps({
    "inputs": {
        "analysisJson": json.dumps({"summary": "showcase preflight", "detections": []}, ensure_ascii=False),
        "inspectionRecordJson": json.dumps({"records": []}, ensure_ascii=False),
        "localModelJson": "{}",
        "buildingContextJson": json.dumps({"buildingId": "showcase-preflight"}, ensure_ascii=False),
    },
    "response_mode": "blocking",
    "user": "urban-safe-showcase-preflight"
}, ensure_ascii=False))
PY
)"

response_file="${RUN_DIR}/preflight/dify-review-assist-probe.json"
...
echo "Dify Review Assist 最小探针：HTTP ${http_code}"
...
if str(status).lower() != "succeeded":
    print("[FAIL] Dify 工作流本身未成功；请依据上方 error 检查 Dify 运行日志对应节点。")
    print(f"响应文件：{path}")
    raise SystemExit(1)
print("[PASS] Dify Review Assist 最小工作流 SUCCEEDED")
print(f"响应文件：{path}")
PY
