...
class YoloXBuildingDefectAdapter:
    ...
    def __init__(self, manifest: YoloXModelManifest, session: Any | None = None, cuda_device_id: int = 0) -> None:
    ...
    def model_info(self) -> ModelBrief:
        return ModelBrief(modelId=self._manifest.model_id, modelName=self._manifest.model_name, version=self._manifest.version)
    ...
    def predict(self, image: DecodedImage) -> tuple[Applicability, list[DetectionItem]]:
        ...
        try:
            ...
            tensor, ratio = letterbox_rgb(pil, self._manifest.input.width, self._manifest.input.height, self._manifest.input.pad_value)
        except (OSError, ValueError) as ex:
            raise ModelUnavailableError("YOLOX 图片预处理失败") from ex
        try:
            outputs = self._session.run([self.OUTPUT_NAME], {self.INPUT_NAME: tensor[None, ...]})
        except Exception as ex:
            raise ModelUnavailableError("YOLOX CUDA 推理失败") from ex
        output = _single_yolox_output(outputs, self._expected_output_shape())
        try:
            boxes, scores, class_ids = decode_yolox(output, (self._manifest.input.height, self._manifest.input.width), len(self._manifest.classes))
        except ValueError as ex:
            raise ModelUnavailableError("YOLOX 输出解码失败") from ex
        finite = np.isfinite(scores) & np.isfinite(boxes).all(axis=1) & (scores >= self._manifest.thresholds.score)
        ...
        keep_local = multiclass_nms(candidate_boxes, candidate_scores, candidate_classes, self._manifest.thresholds.nms_iou, self._manifest.thresholds.maximum_detections)
        ...
        for local_index in keep_local:
            ...
            if class_index < 0 or class_index >= len(self._manifest.classes):
                continue
            normalized = to_normalized_xywh(candidate_boxes[local_index], ratio=ratio, source_width=image.width, source_height=image.height)
            if normalized is None:
                continue
            x, y, width, height = normalized
            model_class = self._manifest.classes[class_index]
            detections.append(DetectionItem(sequence=len(detections) + 1, classCode=model_class.code, className=model_class.name, confidence=float(np.clip(candidate_scores[local_index], 0.0, 1.0)), boundingBox=BoundingBox(x=x, y=y, width=width, height=height, coordinateType=CoordinateType.NORMALIZED_XYWH)))
        if not detections:
            return Applicability.NO_DEFECT_FOUND, []
        return Applicability.APPLICABLE, detections

    def _expected_output_shape(self) -> tuple[int, int, int]:
        return _expected_output_shape(self._manifest)
...
def _create_yolox_session(manifest: YoloXModelManifest, cuda_device_id: int = 0):
    ...
    try:
        ...
        session = ort.InferenceSession(str(manifest.weight_path), sess_options=session_options, providers=providers)
...
def _validate_cuda_graph(session: Any) -> None:
    ...
    for assignment in assignments:
        ...
        if ep_name == CPU_EXECUTION_PROVIDER:
            raise ModelUnavailableError("检测到 CPUExecutionProvider 图分配，拒绝启动 YOLOX 真实模型")
...
def _warm_up_yolox_session(session: Any, manifest: YoloXModelManifest) -> None:
    warmup = np.zeros((1, 3, manifest.input.height, manifest.input.width), dtype=np.float32)
    outputs = session.run([YoloXBuildingDefectAdapter.OUTPUT_NAME], {YoloXBuildingDefectAdapter.INPUT_NAME: warmup})
...
def _expected_output_shape(manifest: YoloXModelManifest) -> tuple[int, int, int]:
    candidate_count = sum((manifest.input.height // stride) * (manifest.input.width // stride) for stride in (8, 16, 32))
...
def _matches_shape(actual: Any, expected: tuple[int, ...]) -> bool:
    if actual is None:
        return False
    try:
        return tuple(int(value) for value in actual) == expected
