...
class FakeAdapter:
    ...
    def model_info(self):
        return ModelBrief(modelId=self.manifest.model_id, modelName=self.manifest.model_name, version=self.manifest.version)
...
def _write_crack_manifest(root: Path) -> None:
    ...
    payload = {
        "schemaVersion": 1, "modelId": "AI-CRACK-001", "modelName": "Crack", "version": "1.0.0",
        "status": "APPROVED", "task": "CRACK_SEGMENTATION", "adapter": "onnx-crack-segmentation-v1",
        "weightFile": "model.onnx", "weightSha256": hashlib.sha256(weight.read_bytes()).hexdigest(),
        ...
        "license": "MIT", "approvedBy": "test", "approvedAt": "2026-08-15T00:00:00Z",
    }
...
def _write_yolox_manifest(root: Path) -> None:
    ...
    classes = [("CRACK", "裂缝"), ("SPALLING", "剥落"), ("EXPOSED_REBAR", "钢筋外露"), ("CORROSION", "锈蚀"), ("WATER_SEEPAGE", "渗水"), ("EFFLORESCENCE", "泛碱"), ("WALL_DAMAGE", "墙体破损")]
    payload = {
        "schemaVersion": 1, "modelId": "AI-BUILDING-YOLOX-001", "modelName": "YOLOX-S 建筑病害检测", "version": "1.0.0",
        "status": "APPROVED", "task": "BUILDING_DEFECT_DETECTION", "adapter": "yolox-building-defect-v1",
        "weightFile": "model.onnx", "weightSha256": hashlib.sha256(weight.read_bytes()).hexdigest(),
        "source": {"type": "TRAINED_FROM_YOLOX", "repository": "https://github.com/Megvii-BaseDetection/YOLOX", "revision": "a" * 40, "license": "Apache-2.0"},
        ...
        "license": "Apache-2.0", "approvedBy": "test", "approvedAt": "2026-08-15T00:00:00Z",
    }
    (package / "manifest.json").write_text(json.dumps(payload, ensure_ascii=False), encoding="utf-8")
...
def _write_catalog(root: Path) -> None:
    payload = {
        "schemaVersion": 1, "runtime": "CUDA_ONLY", "defaultModelId": "AI-CRACK-001",
        "models": [
            {"modelId": "AI-CRACK-001", "version": "1.0.0", "manifestPath": "AI-CRACK-001/1.0.0/manifest.json", "enabled": True},
            {"modelId": "AI-BUILDING-YOLOX-001", "version": "1.0.0", "manifestPath": "AI-BUILDING-YOLOX-001/1.0.0/manifest.json", "enabled": True},
        ],
    }
...
def test_default_registry_factory_contains_yolox_without_enabling_candidate(monkeypatch, tmp_path: Path):
    registry = ModelRegistry(_settings(monkeypatch, tmp_path))
...
def test_registry_routes_yolox_exactly_without_changing_existing_default(monkeypatch, tmp_path: Path):
    ...
    registry = ModelRegistry(_settings(monkeypatch, tmp_path), adapter_factories={"onnx-crack-segmentation-v1": FakeAdapter, "yolox-building-defect-v1": FakeAdapter})

    assert registry.current_model_info().modelId == "AI-CRACK-001"
    assert registry.resolve(InferenceMode.REAL, "AI-BUILDING-YOLOX-001").model_info().modelId == "AI-BUILDING-YOLOX-001"
    ...
    assert info.supportedDefects == ["crack", "spalling", "exposed_rebar", "corrosion", "water_seepage", "efflorescence", "wall_damage"]
