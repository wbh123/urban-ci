...
class FeedbackControllerClosureBoundaryTest {
    ...
    private final FeedbackController controller = new FeedbackController(
            ...
            mock(FeedbackManagementQueryService.class),
            mock(FeedbackClosureService.class));

    @Test
    void genericStatusEndpointCannotSkipRectificationEvidence() {
        assertThatThrownBy(() -> controller.updateStatus(
                UUID.randomUUID(),
                Map.of("status", "RESOLVED", "handlingSummary", "绕过整改证据")))
                .isInstanceOf(InvalidRequestException.class)
                .hasMessageContaining("整改");
    }

    @Test
    void genericStatusEndpointCannotSkipReinspection() {
        assertThatThrownBy(() -> controller.updateStatus(
                UUID.randomUUID(),
                Map.of("status", "CLOSED", "handlingSummary", "绕过复查复验")))
                .isInstanceOf(InvalidRequestException.class)
                .hasMessageContaining("复查复验");
    }
}
