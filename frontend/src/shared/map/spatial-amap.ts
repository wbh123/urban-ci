...
export function createSpatialAmapDriver(
  ...
  function sync(input: SpatialAmapSyncInput): void {
    ...
    input.communityPoints
      ...
      .forEach((point) => {
        ...
        if (CircleMarker && showOfficialBuildings) {
        ...
        const marker = CircleMarker
          ? new CircleMarker({ ...pointStyle(point, selected, theme) })
          : showOfficialBuildings
  ...
  function resolveBuildingHighlightPaths(
    ...
    return [resolvedPath ?? coordinateFence(selection.longitude, selection.latitude)]
  }
