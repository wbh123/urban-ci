...
describe('spatial AMap community point focus', () => {
  it('keeps overview community points as visible concentric status circles and restores overview after focus', async () => {
    ...
    expect(circles[0]!.options.radius).toBe(10)
    expect(circles[0]!.options.strokeColor).toBe('#64748b')
    expect(circles[0]!.options.center).toEqual([114.305, 30.585])
    expect(circles[1]!.options.radius).toBe(6)
    expect(circles[1]!.options.fillColor).toBe('#ff941f')
    circles[1]!.emit('click')
