...
def load_candidate_profile(path: str | Path) -> dict[str, Any]:
    ...
    if payload.get("task") != "BUILDING_DEFECT_DETECTION" or payload.get("adapter") != "yolox-building-defect-v1":
        raise ValueError("YOLOX candidate profile 任务或适配器不匹配")
    classes = payload.get("classes")
    if not isinstance(classes, list) or tuple(item.get("code") for item in classes if isinstance(item, dict)) != YOLOX_BUILDING_DEFECT_CODES:
    ...
    _ratio(thresholds.get("score"), "thresholds.score")
    _ratio(thresholds.get("nmsIou"), "thresholds.nmsIou")
...
def inspect_yolox_session(session: Any, profile: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"executionProvider": CUDA_EXECUTION_PROVIDER, "inputName": "images", "outputName": "output", "inputShape": expected_input, "outputShape": expected_output}
...
def verification_report(profile: dict[str, Any], weight_path: str | Path, contract: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"modelId": profile["modelId"], "targetVersion": profile["targetVersion"], "weightFile": weight.name, "weightSha256": _sha256(weight), "runtimeContractPassed": True, "runtimeContract": contract, "statusRemains": "CANDIDATE", "eligibleForApprovalReview": True, "approvalRequirements": list(profile.get("approvalRequirements", []))}
...
def verify_onnx(profile_path: str | Path, weight_path: str | Path, cuda_device_id: int = 0) -> dict[str, Any]:
    ...
    disable_fallback()
    return verification_report(profile, weight_path, inspect_yolox_session(session, profile))
