...
describe('ConsoleSpatialMapPage production integration', () => {
  ...
  it('uses risk rows for the business list even when a building has no verified polygon', () => {
    expect(source).toContain('const displayRiskBuildings = computed(() => visibleRiskBuildings.value.filter')
    expect(source).toContain('v-for="item in displayRiskBuildings"')
