...
describe('data wall community status aggregation', () => {
  ...
  it('keeps a community usable when any current assessment exists', () => {
    expect(source).toContain("if (freshness.includes('CURRENT')) return 'CURRENT'")
    expect(source).toContain("if (freshness.includes('STALE')) return 'STALE'")
