    ...
    <el-card shadow="never" class="question-card">
      <template #header>
        <div class="card-title">
          <div>
            ...
            <span>只检索当前账号和业务范围允许访问的已审核知识片段，证据不足时拒答。</span>
          </div>
      ...
      <el-form label-position="top" @submit.prevent="submitQuestion">
        ...
        <div class="scope-grid">
          <el-form-item label="治理对象范围（可选）" class="scope-selector-item">
            <SpatialObjectSelector
              v-model:community-id="form.communityId"
              v-model:building-id="form.buildingId"
              mode="both"
            />
          </el-form-item>
    ...
    <el-card v-if="answer" shadow="never" class="answer-card">
      <template #header>
        <div class="answer-header">
          <div>
            <strong>{{ refused ? '受控拒答' : 'AI 回答' }}</strong>
      ...
      <section class="evidence-state" aria-label="证据充分性">
        <strong>证据充分性</strong>
      ...
      <el-collapse class="technical-details">
        <el-collapse-item title="专业技术详情" name="technical">
          <dl class="trace-grid">
...
.scope-grid { display: grid; grid-template-columns: minmax(0, 1fr) minmax(130px, .25fr); align-items: end; gap: 14px; }
.scope-selector-item { min-width: 0; }
...
.evidence-state { display: flex; align-items: center; gap: 10px; margin: 0 0 18px; }
.evidence-state strong { font-size: 14px; }
...
.trace-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px; margin: 4px 0 8px; }
