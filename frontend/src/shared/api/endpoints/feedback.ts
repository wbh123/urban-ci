...
export interface PublicFeedbackBuilding {
  buildingId: string
  buildingCode: string
  buildingName: string
  address?: string
...
export interface CreateFeedbackPayload {
  ...
  locationText?: string
  feedbackChannel?: Exclude<FeedbackChannel, 'WEB'>
}
...
export interface FeedbackImageUploadResult extends FeedbackImage {
  imageCount: number
...
export interface FeedbackManagementRow extends Omit<PublicFeedbackDetail, 'events' | 'images' | 'disclaimer' | 'maxImageCount'> {
  communityId: string
  buildingId?: string
  imageCount: number
...
export function listPublicFeedbackCommunities(): Promise<PublicFeedbackCommunity[]> {
  return apiGet('/api/v1/public/feedback/communities')
}

export function listPublicFeedbackBuildings(communityId: string): Promise<PublicFeedbackBuilding[]> {
  return apiGet(`/api/v1/public/feedback/communities/${communityId}/buildings`)
}

export function createPublicFeedback(payload: CreateFeedbackPayload): Promise<FeedbackCreatedResult> {
  return apiPost('/api/v1/public/feedback/reports', payload)
}

export function uploadPublicFeedbackImage(
  reportCode: string,
  trackingSecret: string,
  file: File,
): Promise<FeedbackImageUploadResult> {
  const formData = new FormData()
  formData.append('trackingSecret', trackingSecret)
  formData.append('file', file, file.name)
  return apiPost(`/api/v1/public/feedback/reports/${encodeURIComponent(reportCode)}/images`, formData)
}

export function listPublicFeedbackImages(
  reportCode: string,
  trackingSecret: string,
): Promise<FeedbackImage[]> {
  return apiGet(`/api/v1/public/feedback/reports/${encodeURIComponent(reportCode)}/images`, {
    trackingSecret,
  })
}

export async function fetchPublicFeedbackImageBlobUrl(
  reportCode: string,
  trackingSecret: string,
  assetId: string,
): Promise<string> {
  const response = await httpClient.get<Blob>(
    `/api/v1/public/feedback/reports/${encodeURIComponent(reportCode)}/images/${assetId}/content`,
    {
...
export function trackPublicFeedback(reportCode: string, trackingSecret: string): Promise<PublicFeedbackDetail> {
...
export function listFeedbackImages(reportId: string): Promise<FeedbackImage[]> {
  return apiGet(`/api/v1/feedback/reports/${reportId}/images`)
}

export function createManualFeedback(payload: CreateFeedbackPayload): Promise<FeedbackCreatedResult> {
  return apiPost('/api/v1/feedback/reports/manual', payload)
}
