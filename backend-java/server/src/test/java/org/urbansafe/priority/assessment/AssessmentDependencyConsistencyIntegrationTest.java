...
/** 验证评分结果链必须完整且依赖状态一致，才能作为当前结果和正式排名。 */
class AssessmentDependencyConsistencyIntegrationTest extends PostgreSqlIntegrationTestBase {
    ...
    @Test
    void partialAssessmentChainIsStaleInsteadOfCurrent() {
        UUID buildingId = createBuilding("PARTIAL");
        UUID completenessRuleId = ruleId("COMPLETENESS", "COMPLETENESS-V1.1");
        insertCompleteness(buildingId, completenessRuleId, "CURRENT", null);
    ...
    @Test
    void rankingExcludesCurrentPriorityLinkedToStaleRisk() {
        UUID buildingId = createBuilding("STALE-DEPENDENCY");
        UUID completenessRuleId = ruleId("COMPLETENESS", "COMPLETENESS-V1.1");
        ...
        UUID riskId = insertRisk(
                buildingId, riskRuleId, completenessId, "STALE", "RULE_CHANGED:COMPLETENESS-V1.1");
        insertRenewal(buildingId, renewalRuleId, riskId, "CURRENT", null, "ALL");
    ...
    private UUID createBuilding(String prefix) {
        ...
        jdbcTemplate.update("""
                ...
                """, communityId, "C-" + prefix + "-" + suffix, "依赖一致性测试小区-" + suffix);
        jdbcTemplate.update("""
                ...
                VALUES (?, ?, ?, ?, '测试路 1 号', 1990, 'BRICK_CONCRETE', 6, 1200,
                        10, 30, 'ACTIVE', '{}'::jsonb)
                """, buildingId, communityId, "B-" + prefix + "-" + suffix, "依赖一致性测试楼-" + suffix);
        return buildingId;
    }
    ...
    private UUID insertRisk(
        ...
        jdbcTemplate.update("""
                ...
                VALUES (?, ?, ?, 'TEST-V1', ?, ?, 64.00, 58.00, 70.00, 'HIGH',
                        ...
                        '{"topFactors":[],"excludedEvidence":[],"missingData":[],"recommendations":["人工复核"]}'::jsonb,
    ...
    private void insertRenewal(
        ...
        jdbcTemplate.update("""
                ...
                VALUES (?, ?, ?, ?, 'TEST-V1', 66.00, 'P2', 1,
                        '{"scopeType":"ALL"}'::jsonb, ?,
                        '{"factors":[],"recommendations":["纳入更新评估"]}'::jsonb,
