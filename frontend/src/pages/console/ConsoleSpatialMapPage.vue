...
const selectedCommunityName = computed(() => selectedCommunityId.value
  ...
  : '全部小区')
const drawerSummary = computed<BuildingSummaryView | null>(() => {
  const projection = selectedProjection.value
  if (!projection) return null
  return {
    buildingId: projection.feature.id,
    buildingCode: projection.feature.properties.entityCode,
    buildingName: projection.feature.properties.name,
    communityName: projection.risk?.communityName || selectedCommunityName.value,
    spatialStatus: 'VERIFIED',
  }
})
const drawerRisk = computed<BuildingRiskSummaryView>(() => {
  const assessment = detailAssessment.value
  const mapRisk = selectedProjection.value?.risk
  const priority = assessment?.renewalPriorities?.[0]
  if (!assessment && !mapRisk) return { freshness: 'NO_RESULT' }
  return {
    freshness: assessment?.freshness ?? mapRisk?.freshness ?? 'NO_RESULT',
    riskScore: assessment?.risk?.riskScore ?? mapRisk?.riskScore,
    riskLevel: assessment?.risk?.riskLevel ?? mapRisk?.riskLevel,
    confidenceScore: assessment?.risk?.confidenceScore ?? mapRisk?.confidenceScore,
    completenessScore: assessment?.completeness?.completenessScore ?? mapRisk?.completenessScore,
    priorityScore: priority?.priorityScore ?? mapRisk?.priorityScore,
    priorityLevel: priority?.priorityLevel ?? mapRisk?.priorityLevel,
    needManualReview: assessment?.risk?.needManualReview ?? mapRisk?.needManualReview,
...
<template>
  <section class="spatial-page">
    ...
    <el-drawer v-model="detailVisible" size="560px" title="楼栋空间详情">
      <div v-if="detailLoading">正在读取楼栋业务数据…</div>
      <template v-else-if="selectedProjection">
        <BuildingSummaryCard v-if="drawerSummary" :summary="drawerSummary" compact />
        <el-tabs v-model="activeDetailTab" class="drawer-tabs">
          <el-tab-pane label="概览" name="overview">
            <RiskSummaryPanel :summary="drawerRisk" />
          </el-tab-pane>
...
.drawer-tabs{margin-top:var(--usp-space-3)}.score-row,.feedback-card{display:grid;gap:6px;padding:12px;margin-bottom:8px;border:1px solid var(--usp-color-border);border-radius:8px}.score-row{grid-template-columns:1fr auto}.score-row small{grid-column:1/-1}
