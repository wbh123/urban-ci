...
import riskSnapshotSource from '@/components/workbench/WorkbenchRiskSnapshot.vue?raw'
import mapPanelSource from '@/components/workbench/WorkbenchMapPanel.vue?raw'
import metricCardSource from '@/components/workbench/WorkbenchMetricCard.vue?raw'
import todoPanelSource from '@/components/workbench/WorkbenchTodoPanel.vue?raw'
import trendPanelSource from '@/components/workbench/WorkbenchTrendPanel.vue?raw'
...
describe('competition dashboard first-screen enhancement', () => {
  it('loads the existing real risk overview only for authorized admin and government workspaces', () => {
    expect(dashboardSource).toContain('getRiskOverview')
    expect(dashboardSource).toContain('workspace.value.enableRisk')
  ...
  it('renders real risk summary fields instead of hard-coded demo statistics', () => {
    expect(riskSnapshotSource).toContain('buildingCount')
    expect(riskSnapshotSource).toContain('assessedBuildingCount')
    expect(riskSnapshotSource).toContain('highRiskCount')
    expect(riskSnapshotSource).toContain('highPriorityCount')
    expect(riskSnapshotSource).toContain('riskDistribution')
  })

  it('keeps risk overview failure as a local graceful degradation', () => {
    expect(dashboardSource).toContain('riskOverviewError')
    expect(riskSnapshotSource).toContain('风险态势暂未加载')
  })

  it('visually distinguishes module cards, priority todos and the current risk map summary', () => {
    expect(metricCardSource).toContain('metric-badge')
    expect(metricCardSource).toContain('data-kind')
    expect(todoPanelSource).toContain('todo-item--high')
    expect(todoPanelSource).toContain('优先处理')
    expect(mapPanelSource).toContain('map-stats')
    expect(mapPanelSource).toContain('高风险')
    expect(mapPanelSource).toContain('building--two')
  })

  it('uses real risk distribution when available and a non-numeric workflow when unavailable', () => {
    expect(trendPanelSource).toContain('riskDistribution')
    expect(trendPanelSource).toContain('发现与受理')
    expect(trendPanelSource).toContain('核查与复核')
    expect(trendPanelSource).toContain('入图与治理')
  })
})
