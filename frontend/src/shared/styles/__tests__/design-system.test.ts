...
describe('R1 设计系统基线', () => {
  it('Design Token 覆盖品牌、风险、排版、间距、圆角、阴影和布局尺寸', () => {
    ...
    for (const token of [
      '--usp-color-primary', '--usp-color-primary-strong', '--usp-color-risk-low',
      '--usp-color-risk-medium', '--usp-color-risk-high', '--usp-color-text',
      '--usp-color-text-secondary', '--usp-color-bg', '--usp-color-surface',
      '--usp-space-1', '--usp-space-6', '--usp-radius-sm', '--usp-radius-lg',
      '--usp-shadow-sm', '--usp-shadow-lg', '--usp-console-aside-width',
      '--usp-console-aside-collapsed-width', '--usp-header-height',
  ...
  it('全局圆角覆盖原生按钮、选择框弹层与组合输入控件', () => {
    ...
    for (const selector of [
      'button:not(.el-button)', '.el-select-dropdown', '.el-dropdown-menu',
      '.el-picker-panel', '.el-segmented', '.el-input-number .el-input__wrapper',
